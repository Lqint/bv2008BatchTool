...
class MainWindow(QMainWindow):
    ...
    def show_start_notice(self) -> None:
        ...
        notice = (
            ...
            + _p("2. 本工具为 v3.2 版本，暂为最新稳定版，如志愿北京平台规则不发生变化，不会更新。")
            + _p("3. 本工具大致链路：根据姓名和身份证号(如有)在团体内查询志愿者id->若未查到且已填写身份证号，将其加入团体->使用uid加入对应岗位->录入志愿时长。")
            + _p("4. " + red + "您仍需手动在“志愿北京”网站上完成创建项目、创建子项目、创建活动。" + end + "本工具只用于提高招募志愿者和录入时长的效率。")
            + _p(red + "5. 请按左侧操作区的顺序依次完成登录、填写活动参数、获取活动信息、上传表格、选择起始日期、启动批量录入。活动 ID 需自行从“志愿北京”网站获取，获取方式请参考配套文档。表格需包含列：姓名、身份证号、岗位、时长、备注。您可使用提供的模板.xlsx。建议填写身份证号，如不填，您需让志愿者自行加入团体或活动；备注内容为空时，程序会自动生成“日期+岗位+服务+时长h”。时长只能填写整数或 .5 小数。岗位列内容必须与“志愿北京”平台一致，也就是与程序右上方自动获取的岗位列表一致。" + end)
            + _p("6. 图片证明材料仅支持 jpg/png 格式；未选择证明材料时，程序会自动使用 1x1 PNG 占位图。")
            + _p(red + "7. 每日最多录入 10 小时，程序会自动按天拆分，并根据剩余活动日期计算可行性，若超出可录入范围，将跳过。" + end)
