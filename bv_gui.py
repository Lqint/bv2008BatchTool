...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("志愿北京时长批量录入v3.1")
    ...
    def show_start_notice(self) -> None:
        ...
        notice = (
            ...
            + _p("2. 本工具为 v3.1 版本，暂为最新稳定版，如志愿北京平台规则不发生变化，不会更新。")
            ...
            + _p(red + "5. 请按左侧操作区的顺序依次完成登录、填写活动参数、获取活动信息、上传表格、选择起始日期、启动批量录入。活动 ID 需自行从“志愿北京”网站获取，获取方式请参考配套文档。表格需包含列：姓名、身份证号、岗位、时长、备注。您可使用提供的模板.xlsx。身份证号可不填，但您需让志愿者自行加入团体或活动；备注内容为空时，程序会自动生成“日期+岗位+服务+时长h”。时长只能填写整数或 .5 小数。岗位列内容必须与“志愿北京”平台一致，也就是与右侧自动获取的岗位列表一致。" + end)
